...
def procTcp4(pkt):
    ...
    pkt.tcp_datalength = (pkt.ip_maxoffset - pkt.tcp_data_offset)/8
    # find TCP client
...
class Ntw:
    ...
    def sendUdp4(self, tgt_ip, tgt_port, data, src_port=0):
        ...
        if tgtMac is None:
            if (LOG_LEVEL_ERROR <= self.logUdp4Level):
    ...
    def sendTcp4(self, tgt_ip, tgt_port, src_port, data, tcp_seq_num, tcp_ack_num, flags, tcp_window_size = 1024, tcp_options_raw = bytes(0)):
        ...
        if tgtMac == None:
            print("invalid MAC, while sending tcp")
            return -1
...
def main():
    ...
    ntw.setIPv4([192,168,68,117], [255,255,255,0], [192,168,40,1])
